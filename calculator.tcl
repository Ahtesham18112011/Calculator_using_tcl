...
proc evaluate {} {
    global numberString
    regsub {^0+(?!$)} $numberString "" numberString
...
proc sqrt {} {
    global numberString
    if {[regexp {^(\d+)\s*√$} $numberString -> num]} {
        catch {
            set numberString [expr {sqrt($num)}]
        } err
    }
}
...
button .converter.cmToIn -text "cm → in" -command {
...
button .converter.inToCm -text "in → cm" -command {
    set val [.converter.input get]
    catch {
        .converter.output delete 0 end
        .converter.output insert 0 [expr {$val * 2.54}]
...
button .converter.ctoF -text "C → F" -command {
    set val [.converter.input get]
        ...
        .converter.output delete 0 end
        .converter.output insert 0 [expr {($val * 9/5) + 32.0}]
    }
}
button .converter.ftoC -text "F → C" -command {
    set val [.converter.input get]
        ...
        .converter.output delete 0 end
        .converter.output insert 0 [expr {($val - 32) * 5/9.0}]
    }
}



grid .converter.input -row 1 -column 0 -columnspan 2 -pady 5
grid .converter.output -row 2 -column 0 -columnspan 2 -pady 5
...
grid .converter.back -row 6 -column 0 -padx 5 -pady 10
...
button .converter.cmtom -text "cm → m" -command {
    set val [.converter.input get]
    catch {
        .converter.output delete 0 end
        .converter.output insert 0 [ expr {$val / 100.0}]
    }
}

grid .converter.cmtom -row 5 -column 1 -padx 5 -pady 10

button .converter.mtocm -text "m → cm" -command {
    set val [.converter.input get]
    catch {
        .converter.output delete 0 end
        .converter.output insert 0 [expr {$val * 100.0}]
...
grid .converter.mtocm -row 5 -column 0 -padx 5 -pady 10

button .converter.clear -text "clear" -command {
    .converter.input delete 0 end
    .converter.output delete 0 end
    .converter.input insert 0 ""
    .converter.output insert 0 ""
}

grid .converter.clear -row 6 -column 1 -padx 5 -pady 10
